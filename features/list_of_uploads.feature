Feature: View a list of batch files
  ...
  Background:
    ...
    Given I have a user "supervisor@intersect.org.au" with role "Data Provider Supervisor" and clinic "RPA"
    Given I have a user "admin@intersect.org.au" with role "Administrator" and clinic "RPA"
    ...
    And I have batch uploads
      | survey    | created_by                     | created_at       | status      | message   | clinic | summary report | detail report | file_file_name | record_count | year_of_registration |
  ...
  Scenario Outline: View a list of batch uploads - data provider/data provider supervisor only sees own clinic
